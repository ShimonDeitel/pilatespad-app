    ...
    private func row(_ entry: ClassEntry) -> some View {
        HStack(alignment: .top, spacing: 12) {
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text("Duration (min): \(entry.value1)  \u{00B7}  Intensity (1-5): \(entry.value2)")
                    .font(.caption)
...
struct EntrySheet: View {
    ...
    @State private var option: String = PilatespadOptions.all.first ?? ""
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section("Apparatus") {
                    Picker("Apparatus", selection: $option) {
                        ForEach(PilatespadOptions.all, id: \.self) { t in
                            Text(t).tag(t)
                        }
                    }
                    .pickerStyle(.menu)
                ...
                Section("Intensity (1-5): \(Int(value2))") {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        if var existing = entry {
                            existing.apparatus = option
                            existing.value1 = Int(value1)
                            existing.value2 = Int(value2)
                            existing.note = note
                            store.update(existing)
                        } else {
                            store.add(apparatus: option, value1: Int(value1), value2: Int(value2), note: note)
                        }
                        dismiss()
                    }
